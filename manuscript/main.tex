...
partition and the finite-plus-infinite Krawczyk inclusion. Five light quotient
classes are identified as persistent rigid rational complete-intersection
curves, as a supporting geometric result rather than part of the central proof.
\end{abstract}
...
real Jacobian, and the final Krawczyk operator. The novelty claimed here is
this combined certificate on an explicit multi-parameter compact
Calabi--Yau model; no separate claim of priority is made for its individual
algebraic, mirror-symmetry, or interval-arithmetic ingredients. This is a
deliberately narrower result than a proof of a complete physical vacuum.
...
\input{generated_certificate_table}

\section{Logical structure and proof inputs}

The proof has two logically distinct layers.  The generator layer evaluates
the analytic period and F-term formulae with Arb interval arithmetic, performs
the exact Chow and monoid computations, and serializes outward-rounded
witnesses.  The portable checker layer authenticates the three packaged proof
attachments, recomputes the exact low-degree partition and canonical digests,
recomputes the twelve rational tail propagations, and verifies the final
strict Krawczyk inequalities.  It deliberately does not reimplement Arb,
Normaliz, or CYTools.

Accordingly, the trusted base consists of four explicit input classes:
(i) the toric model, symplectic convention, flux vectors, and analytic period
formulae stated below; (ii) exact Chow and monoid arithmetic for the finite
degree ledger; (iii) the Arb enclosures for the mirror map, finite F-system,
and real Jacobian on the declared boxes; and (iv) the elementary rational
arithmetic of the portable checker.  Approximate centers and inverse matrices
are only candidate witnesses.  They acquire mathematical force only through
strict interval inclusion.  As in other computer-assisted proofs, the result
also relies on the correct execution of the named arithmetic libraries and
the underlying hardware; the release is not a formally verified
implementation of those libraries.

The archived hashes fix the exact witnesses used for the theorem.  The three
packaged proof attachments are checked in every standalone run.  The further
upstream provenance manifest contains 16 hashes and can be checked against an
author snapshot with the optional \texttt{--source-root} mode; those upstream
files are not silently counted as authenticated in the standalone mode.
...
witness $C$, define the Krawczyk operator \cite{Breiding2024,Moore2009}
\begin{equation}
...
The radius $10^{-30}$ is an exact certification scale, not a physical error
bar and not an assumed accuracy of the root.  It was selected after a
high-precision approximate solve and is admitted only because the subsequent
box calculation proves strict inclusion.  This a posteriori choice does not
enter as a hypothesis of the Krawczyk theorem.  No assertion is made that the
same preconditioner certifies a substantially larger box.  In particular, the
full finite sector moves the earlier four-term center by about
$2.93\times10^{-20}$, so recentering is essential before the radius-$10^{-30}$
...
The complete finite sector is included in the Arb evaluator and certified in
the new radius-$10^{-30}$ box around the recentered root. The inverse mirror
...
It authenticates the packaged proof attachments by SHA-256, recomputes strict
scalar and componentwise inclusion inequalities, checks scope constraints,
and fails if the finite-sector or degree-partition witnesses are absent.  The
heavy generators use Arb, exact rational Chow arithmetic, Normaliz, and
CYTools.  Their serialized interval enclosures remain part of the trusted
input; the lightweight checker is an independent last-mile checker, not a
second evaluation of the analytic system.
...
\path{data/certificate.json} and
\path{data/recentered_tail_propagation.json}. It is therefore a build
artifact rather than a second manually maintained statement of the numerical
claim.

The exact public release used for this article is version v1.0.5, archived
under the version DOI
\href{https://doi.org/10.5281/zenodo.21906462}{10.5281/zenodo.21906462}.
The concept DOI
\href{https://doi.org/10.5281/zenodo.21906026}{10.5281/zenodo.21906026}
resolves to the latest release; the source is maintained in the
\href{https://github.com/megazord78-pixel/interval-flux-vacuum-5-81}
{public GitHub repository}.

\section*{Use of AI-assisted tools}

OpenAI Codex was used for software-engineering assistance in isolating the
portable verifier, constructing reproducibility and mutation tests, checking
release packaging, and editing the English and presentation of the
manuscript.  It was not treated as a source of mathematical or physical
evidence.  The author selected the claims, inspected the code and generated
artifacts, and accepts full responsibility for the scientific content and the
submitted text.
